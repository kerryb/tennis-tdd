defmodule TennisScorer.GameTest do
  ...
  describe "TennisScorer.Game.score/1" do
    ...
    test "returns advantage when player 2 is up 4–3" do
      assert Game.score(%Game{name_2: "Bob", score_1: 3, score_2: 4}) == "Advantage Bob"
    end

    test "returns advantage when player 2 is one point up and over 3" do
      assert Game.score(%Game{name_2: "Bob", score_1: 5, score_2: 6}) == "Advantage Bob"
    end

    test "returns deuce when tied on 4" do
      assert Game.score(%Game{score_1: 4, score_2: 4}) == "Deuce"
    end

    test "returns deuce when tied on > 4" do
      assert Game.score(%Game{score_1: 6, score_2: 6}) == "Deuce"
    end

    test "returns game when player 1 has 4 and player 2 < 3" do
      assert Game.score(%Game{name_1: "Alice", score_1: 4, score_2: 2}) == "Game Alice"
    end

    test "returns game when player 1 is > 1 point up and over 3" do
      assert Game.score(%Game{name_1: "Alice", score_1: 6, score_2: 4}) == "Game Alice"
    end

    test "returns game when player 2 has 4 and player 2 < 3" do
      assert Game.score(%Game{name_2: "Bob", score_1: 2, score_2: 4}) == "Game Bob"
    end

    test "returns game when player 2 is > 1 point up and over 3" do
      assert Game.score(%Game{name_2: "Bob", score_1: 4, score_2: 6}) == "Game Bob"
    end
  end
end
